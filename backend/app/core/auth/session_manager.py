...
def get_current_user(request: Request, payload: dict = Depends(get_token_payload), db: Session = Depends(get_db)) -> m.User:
    ...
    try:
        ...
        # Si el rol es cliente y el sub no es un int, aceptar como invitado
        role = payload.get("role")
        if role == "cliente":
            return m.User(id=0, email=str(sub), role="cliente")  # type: ignore[arg-type]
    ...
    # Si no existe el usuario pero el rol es 'cliente', devolver usuario invitado temporal
    role = payload.get("role")
    if not user and role == "cliente":
        # Construir usuario invitado temporal
        return m.User(id=uid, email=payload.get("sub", "guest@example.com"), role="cliente")  # type: ignore[arg-type]
    if not user:
